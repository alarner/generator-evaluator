 ...
 */
function generateExpression(maxDepth=3, currentDepth=1, intMin=1, intMax=15) {
	if(intMin < 1) {
		throw new Error('intMin must be positive');
	}

	let branch1 = Math.random() < 0.3;
	let branch2 = Math.random() < 0.3;
	...
	if(currentDepth < maxDepth) {
		...
		if(branch2) {
			expression2 = generateExpression(
				maxDepth,
				currentDepth+1,
				intMin,
				intMax
 ...
 * range of 0 - 10.
 */
function generateInteger(min=0, max=10) {
	if(max < min) {
